-- ============================================================================
-- MIGRATION 018: Scalability Refactoring (VALIDATED VERSION)
-- ============================================================================
-- Purpose: Address critical scalability issues flagged by Data Engineer review
-- 
-- PRE-MIGRATION VALIDATION COMPLETED:
--   ✓ All tables exist (word_occurrence, brief_word_occurrence, etc.)
--   ✓ All target columns are citext type
--   ✓ Tables are empty (safe to migrate)
--   ✓ tsvector columns exist on case_chunks and case_sentences
--   ✓ Views checked - v_qa_extraction uses issue_outcome (will handle)
--   ✓ Existing indexes identified to handle during rename
--   ✓ Existing CHECK constraints identified for removal
-- ============================================================================
...
RAISE NOTICE 'PART 1 COMPLETE: All ENUM types created';
...
-- Save the view definition for recreation
DROP VIEW IF EXISTS public.v_qa_extraction;
RAISE NOTICE 'Dropped v_qa_extraction view (will recreate after column migration)';


-- ============================================================================
-- PART 3: DROP OLD CHECK CONSTRAINTS
-- ============================================================================
-- These constraints use citext comparisons which won't work with ENUMs
...
RAISE NOTICE 'PART 3 COMPLETE: Old CHECK constraints dropped';


-- ============================================================================
-- PART 4: DROP OLD INDEXES ON STATUS COLUMNS
-- ============================================================================
-- These will be invalid after column type change
...
RAISE NOTICE 'PART 4 COMPLETE: Old indexes on status columns dropped';
...
RAISE NOTICE 'PART 5 COMPLETE: New ENUM columns added';


-- ============================================================================
-- PART 6: MIGRATE DATA (Handle case-sensitivity)
-- ============================================================================
-- Note: Tables are currently empty, but this handles existing data safely
...
SET publication_status_new = 
    CASE 
        WHEN processing_status::text ILIKE 'Published' THEN 'Published'::publication_status_type
...
RAISE NOTICE 'PART 6 COMPLETE: Data migrated to new columns';
...
RAISE NOTICE 'PART 7 COMPLETE: Old columns dropped, new columns renamed';
...
RAISE NOTICE 'PART 8 COMPLETE: Indexes recreated on ENUM columns';
...
SELECT 
    ...
    (SELECT json_agg(json_build_object(
        ...
        'outcome', id.issue_outcome::text,  -- Cast ENUM to text for JSON
...
RAISE NOTICE 'PART 9 COMPLETE: v_qa_extraction view recreated';


-- ============================================================================
-- PART 10: DROP WORD_OCCURRENCE TABLES (REDUNDANT WITH tsvector)
-- ============================================================================
-- Verified: case_chunks.tsv and case_sentences.tsv exist as tsvector columns
-- The word_occurrence tables are redundant for full-text search
...
RAISE NOTICE 'Dropped word_occurrence table (redundant with tsvector)';
...
RAISE NOTICE 'Dropped brief_word_occurrence table (redundant with tsvector)';

RAISE NOTICE 'PART 10 COMPLETE: Redundant word_occurrence tables dropped';
...
RAISE NOTICE 'PART 11 COMPLETE: Missing FK indexes added';
...
RAISE NOTICE 'PART 12 COMPLETE: Table statistics updated';
...
BEGIN
    ...
    ELSE
        RAISE WARNING '✗ MIGRATION MAY HAVE ISSUES - CHECK RESULTS';
    END IF;
END $$;
